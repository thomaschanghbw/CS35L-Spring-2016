...
void* ThreadFunction(void *arg)
{
  ...
  const double subsample_dy = halfSamples ? pixel_dy / ((double)halfSamples*2.0)
	    : pixel_dy;
  int end;
  if((int)arg+2*chunkSize < width)
    end = (int)arg+chunkSize;
  else
    end = width;
  ...
  for(int px = (int)arg; px < end; ++px )
	      ...
	      for( int py=0; py<height; ++py )
		{
		   ...
		   */
		  struct Pixel_vals curr_vals;
		  curr_vals.color_one = scaled_color[0];
		  curr_vals.color_two = scaled_color[1];
		  curr_vals.color_three = scaled_color[2];
		  values[px*width + py] = curr_vals;
		}
		    }
	  pthread_exit(0);  
}
...
main( int argc, char **argv )
{
    ...
    pthread_t *ID = (pthread_t*)malloc(sizeof(pthread_t)*nthreads);
    long t;
    chunkSize = width / nthreads;
    int* start = (int*)malloc(sizeof(int)*nthreads);
    values = (struct Pixel_vals*)malloc(sizeof(struct Pixel_vals)*width*height);
    ...
    for(t=0; t<nthreads; t++)
      {
	int rs = pthread_create(&ID[t], 0, ThreadFunction, (void*) start[t]);
	if(rs)
	  {
    ...
    for(t=0; t<nthreads; t++)
      {
	int rs = pthread_join(ID[t], &retVal);
	if(rs)
	  {
    ...
    int xx, yy;
    struct Pixel_vals pvals;
    for(xx=0; xx<width; ++xx)
      {
	for(yy=0; yy< height; ++yy)
	  {
	    pvals = values[xx*width+yy];
	    printf("%.0f %.0f %.0f\n",
		   pvals.color_one,
		   pvals.color_two,
		   pvals.color_three);
